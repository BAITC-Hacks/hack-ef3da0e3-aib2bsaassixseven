...
Текущая схема: сайт → backend приложения → собственная модель на NVIDIA-сервере
команды → локальные результаты в `data/` приложения → дашборд. Временное аудио
удаляется после подтверждённого сохранения результатов; без Docker.
