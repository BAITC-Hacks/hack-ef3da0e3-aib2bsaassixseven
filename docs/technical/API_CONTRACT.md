...
Это контракт v1 для загруженного локального аудио встречи. В ветке `codex/meeting-mvp-stage45` реализованы backend-маршруты встреч, локальный coordinator, review и PDF-экспорт. Сквозной проход через настоящий NVIDIA-сервис и frontend ещё требует интеграции веток команды. Потоковый захват живой встречи в v1 не входит.
...
Маршруты таблицы реализованы в backend ветки `codex/meeting-mvp-stage45`; тесты с GPU используют синтетический результат и fake-клиент.
