...
Это контракт v1 для загруженного локального аудио встречи. В `main` реализованы backend-маршруты встреч, локальный coordinator, review и PDF-экспорт; эта ветка добавляет источник записи вкладки после завершения записи. Сквозной проход через настоящий NVIDIA-сервис и frontend ещё требует интеграции веток команды. Потоковый захват живой встречи в v1 не входит.
...
Для `POST /meetings` обязательны две части формы: `audio` — бинарный файл и `metadata` — JSON-объект с `Content-Type: application/json` либо обычное текстовое поле браузерного `FormData` без `Content-Type`. В `metadata` обязательны `title` (1–120 символов), `meeting_date` (`YYYY-MM-DD`), `timezone` (действительный идентификатор IANA), `participants` (массив из 0–30 уникальных имён по 1–100 символов) и `recording_notice_confirmed: true`; `language_hint` необязателен, по умолчанию `auto`, значения `auto|ru|kk|mixed`. Необязательное `source_kind` принимает только `uploaded_audio` (по умолчанию) или `browser_recording`; `demo_fixture` публичный POST не принимает. Это заявленный пользователем способ получения файла, а не техническое доказательство записи браузером. В ответе источник представлен через `source.kind`, без повторного `source_kind`. Отметка `recording_notice_confirmed` фиксирует подтверждение пользователя об уведомлении участников, но сама по себе не доказывает его получение. `false`, отсутствие обязательного поля, неверная дата или часовой пояс дают `422`. Принимаются `.wav`, `.mp3`, `.m4a`, `.ogg`, `.webm` с проверкой содержимого, не только расширения; для `.webm` требуется контейнер WebM с аудиокодеком Opus. Максимум 100 MiB. Проверенное расширение сохраняется во внутреннем manifest, чтобы при передаче на NVIDIA исходный формат был указан в имени файла и MIME, несмотря на приватное имя локального файла `upload.bin`. Загрузка атомарна: после `202` аудио временно сохранено приложением и задача передачи на NVIDIA записана в manifest. Это ещё не подтверждение, что GPU принял задачу. Если временное сохранение upload и manifest не завершилось, встреча не появляется в списке. Имя загруженного файла не используется как путь хранения. Создание `demo_fixture` не открывается публичным маршрутом: пример заранее подготовлен для демонстрационного аккаунта и возвращается обычными `GET`-маршрутами с меткой источника.
