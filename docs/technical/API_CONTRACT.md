...
Это контракт v1 для загруженной локальной аудио- или видеозаписи встречи. В `main` реализованы backend-маршруты встреч, локальный coordinator, review и PDF-экспорт. Видео нормализуется локально внутри FastAPI приложения: на NVIDIA передаётся только извлечённая первая аудиодорожка. Сквозной проход через настоящий NVIDIA-сервис и frontend ещё требует интеграции веток команды. Потоковый захват живой встречи в v1 не входит.
...
Маршруты таблицы реализованы в backend; тесты с GPU используют синтетический результат и fake-клиент.
...
Для `POST /meetings` обязательны две части формы: `audio` — бинарный файл записи (имя части сохранено для совместимости и допускает аудио или видео) и `metadata` — JSON-объект с `Content-Type: application/json` либо обычное текстовое поле браузерного `FormData` без `Content-Type`. В `metadata` обязательны `title` (1–120 символов), `meeting_date` (`YYYY-MM-DD`), `timezone` (действительный идентификатор IANA), `participants` (массив из 0–30 уникальных имён по 1–100 символов) и `recording_notice_confirmed: true`; `language_hint` необязателен, по умолчанию `auto`, значения `auto|ru|kk|mixed`. Необязательное `source_kind` принимает только `uploaded_audio` (по умолчанию; историческое имя также используется для загруженного видео) или `browser_recording`; `demo_fixture` публичный POST не принимает. Это заявленный пользователем способ получения файла, а не техническое доказательство записи браузером. В ответе источник представлен через `source.kind`, без повторного `source_kind`. Отметка `recording_notice_confirmed` фиксирует подтверждение пользователя об уведомлении участников, но сама по себе не доказывает его получение. `false`, отсутствие обязательного поля, неверная дата или часовой пояс дают `422`.

Входной файл ограничен 100 MiB до декодирования. Audio-only `.wav`, `.mp3`, `.m4a`, `.ogg` и `.webm` проверяются по содержимому, не только по расширению; audio-only WebM должен содержать Opus. `.mp4`, `.mov`, `.mkv`, а также WebM с видеодорожкой принимаются при наличии аудиопотока. Backend запускает локальные `ffprobe`/`ffmpeg` без shell, с закрытыми stdin/stdout/stderr, ограничением 5 секунд для probe, 120 секунд и одним потоком для extraction. Из видео выбирается `0:a:0`, метаданные и главы удаляются, видеопотоки не копируются, результат кодируется в audio-only M4A/AAC. Выход ограничен 100 MiB и повторно проверяется. Исходный staged-видеофайл удаляется до создания встречи; на NVIDIA и в постоянные результаты видео не попадает. Файл без аудиодорожки, повреждённый/подменённый контейнер, timeout, ошибка локального декодера или слишком большой нормализованный результат дают `415 unsupported_media_type`, и встреча не появляется.

Проверенное расширение итогового audio-only payload (`.m4a` после видео-нормализации) сохраняется во внутреннем manifest, чтобы при передаче на NVIDIA формат был указан в имени файла и MIME, несмотря на приватное имя локального файла `upload.bin`. Загрузка атомарна: после `202` нормализованное аудио временно сохранено приложением и задача передачи на NVIDIA записана в manifest. Это ещё не подтверждение, что GPU принял задачу. Если временное сохранение upload и manifest не завершилось, встреча не появляется в списке. Имя загруженного файла не используется как путь хранения. Создание `demo_fixture` не открывается публичным маршрутом: пример заранее подготовлен для демонстрационного аккаунта и возвращается обычными `GET`-маршрутами с меткой источника.
